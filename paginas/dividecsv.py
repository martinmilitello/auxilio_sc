...
if len(sys.argv) == 4:
    ...
    rutasubida = "/var/www/qa/ayudasistemacontableq/auxilio_sc/paginas/"
    
    cabenombrearchivo = "C" + mifecha + fijo + mizona + ".csv" # rutasubida + 
    detanombrearchivo = "D" + mifecha + fijo + mizona + ".csv" #  rutasubida +
    clinombrearchivo  =  "CLI" + mifecha + mihora + "_Z" + mizona + ".csv" #rutasubida +
    
    # print("argumentos:" + mifecha + mihora + mizona)
    ...
    os.chdir('..')  # Subo un nivel dentro del sitio
    os.chdir('subidas')
    current_directory = os.getcwd()
    ...
    try:
        ...
        print("Error: El archivo -> " + cabenombrearchivo +" CSV no existe o no se puede abrir ")
    ...
    try:
        ...
        print("Error: El archivo -> " + detanombrearchivo +" CSV no existe o no se puede abrir ")
    try:   
        cli_df=pd.read_csv(clinombrearchivo, dtype=str,quotechar = '"')
        
    except FileNotFoundError:
        
        print("Error: El archivo -> " + clinombrearchivo +" CSV no existe o no se puede abrir ")
    
    
    # print(cabe_df)
    try:
        demapas_df = pd.read_csv("demapas.csv", dtype=str)   # Archivo CSV revend sin 000000  ------- rutasubida+
    except FileNotFoundError:
        print("Error: El archivo -> " + "demapas.csv" +" no existe o no se puede abrir ")
  ...
  #  print(demapas_df_type)
    ...
    deta_df_in.cantidad=deta_df_in.cantidad.astype(int)
...
#    cli_df.piso=cli_df.piso.fillna(0).astype(int)
    ...
    cabe_in_salida= "C" + mifecha + "_ENXLSX_" + mizona + ".csv"
    deta_in_salida= "D" + mifecha + "_ENXLSX_" + mizona + ".csv"
    cabe_not_salida= "C" + mifecha + "_RESTO_" + mizona + ".csv"
    deta_not_salida= "D" + mifecha + "_RESTO_" + mizona + ".csv"
    cli_salida=  "CLI" + mifecha + "_" + mizona + "_ENXLSX_" + mizona  + ".csv"
