...
# que deja en subidas los txt con las zonas elegidas
...
servidor = '//ms10004.vfc.com.ar/fileserver/grupales'
usuario = 'mmilitello'
contrasena = 'Violetta40'
directorio_compartido = 'grupales'
'''
...
if len(sys.argv) == 3:
    ...
    pidom = sys.argv[2] # Mes de los archivos CSV

    print("Antes de conectarse"+ servidor + directorio_compartido + "---"+usuario+"---")
    try:
        
        with smbclient.SambaClient(server=servidor, username=usuario, password=contrasena) as cliente:
            archivos = cliente.listdir("/")
   ...
   # print (mitrabajo)
    ...
    '''     
    ruta = r'//ms10004.vfc.com.ar/fileserver/grupales/InterfacesSAP/INTPEDIDOS/BACKUP'
    if os.path.exists(ruta):
        print("La ruta existe")
        os.chdir(ruta)  # Cambiar al directorio de trabajo actual a la ruta especificada
    else:
            ...
            for archi1 in glob.glob('C'+str(qfecha6)+'*'+str(puzo).rjust(3, '0')+'.csv'):
                if archi1 and len(archi1)>0:
                    ...
                    if estedia in range (cachodia-3,cachodia+3):
                        ...
                        sale.append([archi,puzo,putot,sumacsvorig,df_sumadispo,lax,lasuma, fecharchi,porce])

    os.chdir(trabajo)  

    dfx = pd.DataFrame(sale, columns=['PSTD','zona','Txt','CSV','disponible','laX','lasuma','fecha','porce'])
    ...
    writer = pd.ExcelWriter(r"totales_txt"+elamd+".xlsx")
